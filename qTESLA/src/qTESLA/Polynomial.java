...
public class Polynomial {
	 ...
	 *******************************************************************/
	private static long montgomery (long number, int q, long qInverse) {// private
	...
	public static int barrett (long number, int q, int barrettMultiplication, int barrettDivision) {
		...
		return (int) (number - ((number * barrettMultiplication) >> barrettDivision) * q);
		...
		for (int numberOfProblem = 1; numberOfProblem < n; numberOfProblem *= 2) {
			...
			for (int jFirst = 0; jFirst < n; jFirst = j + numberOfProblem) {
				...
				for (j = jFirst; j < jFirst + numberOfProblem; j++) {
					...
					destination[destinationOffset + j + numberOfProblem]	=	montgomery (omega * (temporary + (2 * q - destination[destinationOffset + j + numberOfProblem])), q, qInverse);
			...
			for (jFirst = 0; jFirst < Parameter.N_III_P; jFirst = j + numberOfProblem) {
				...
				for (j = jFirst; j < jFirst + numberOfProblem; j++) {
					...
					destination[destinationOffset + j + numberOfProblem]	=	barrettP (
																					montgomery (
																							omega * (2L * Parameter.Q_III_P + temporary - destination[destinationOffset + j + numberOfProblem]),

...
public class Pack {
	...
	public static void encodePublicKeyIIIP (byte[] publicKey, final long[] T, final byte[] seedA, int seedAOffset) {
		...
		for (int i = 0; i < Parameter.N_III_P * Parameter.K_III_P * Parameter.Q_LOGARITHM_III_P / Integer.SIZE; i += Parameter.Q_LOGARITHM_III_P) {
			...
			for (int index = 0; index < Parameter.Q_LOGARITHM_III_P; index++) {
				...
				Common.store32 (publicKey, Integer.SIZE / Byte.SIZE * (i + index), (int) ((T[j + index] >> index) | (T[j + index + 1] << (Parameter.Q_LOGARITHM_III_P - index))));
	...
	public static void decodePublicKeyIIIP (int[] publicKey, byte[] seedA, int seedAOffset, final byte[] publicKeyInput) {
		...
		for (int i = 0; i < Parameter.N_III_P * Parameter.K_III_P; i += Integer.SIZE) {
			...
			publicKey[i + Parameter.Q_LOGARITHM_III_P] = Common.load32 (publicKeyInput, Integer.SIZE / Byte.SIZE * (j + Parameter.Q_LOGARITHM_III_P - 1)) >>> 1;
